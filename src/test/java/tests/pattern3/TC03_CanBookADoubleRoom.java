...
public class TC03_CanBookADoubleRoom extends TestBaseHotel {

    @Test()
    @Description("テスト: グループ部屋の予約ができる")
    public void TC03() {
        ...
        roomReservationPage.inputCheckInDayForSearch(TestConstants.PT3_TC03_DAY_FOR_SEARCH);

        Logger.info("2. ツインボタンを押下");
        roomReservationPage.selectRoomType(RoomType.DOUBLE);

        Logger.info("3. 部屋301のチェックボックスを押下");
        roomReservationPage.findAndPressCheckBoxOfRoom(TestConstants.PT3_TC03_SELECT_ROOM_1);

        Logger.info("4.「チェックイン」に入力");
        Logger.info("5.「チェックアウト」に入力");
        Logger.info("6. 追加ボタンを押下");
        bookingRoomPage.selectDate(TestConstants.PT3_TC03_CHECK_IN_DAY_1, TestConstants.PT3_TC03_CHECK_OUT_DAY_1);
        ...
        addNewPaymentCardPage.addNewCard(TestConstants.PT3_TCO3_DATA_PAYMENT);
        ...
        AssertHelper.checkEqual("カード番号 - **************0000", paymentSuccessPage.getNumberPaymentCard(),
        ...
        AssertHelper.checkTrue(paymentSuccessPage.isNamePaymentCardDisplayedCorrectly(TestConstants.PT3_TCO3_DATA_PAYMENT.getCardName()),
                "前提条件のステップ９の「カード名義」が同じ行で表示されていません。");

        Logger.verify("前提条件のステップ１０の「カード番号」が表示される事。");
        AssertHelper.checkEqual("**************0000", paymentSuccessPage.getNumberPaymentCard(),
                "支払いカード番号「＊＊＊＊＊＊＊＊＊＊＊＊１１１１」が表示されません");

        Logger.verify("ステップ3の部屋を表示されること。");
        AssertHelper.checkEqual(TestConstants.PT3_TC03_SELECT_ROOM_1, paymentSuccessPage.getNameRoomInList(),
                "ステップ3の部屋を表示されていません。");
        ...
        bookedRoomPage.searchBookedRoom(TestConstants.PT3_TC03_CHECK_IN_DAY_1, TestConstants.PT3_TC03_CHECK_OUT_DAY_1);

        Logger.info(TestConstants.PT3_TC03_SELECT_ROOM_1 + "部屋を表示される");
        AssertHelper.checkTrue(bookedRoomPage.isRoomDisplayed(TestConstants.PT3_TC03_SELECT_ROOM_1),
                TestConstants.PT3_TC03_SELECT_ROOM_1 + "部屋を表示されません");
